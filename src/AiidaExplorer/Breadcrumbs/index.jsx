...
export default function Breadcrumbs({ trail, onClick, maxItems = 10 }) {
  ...
  return (
    <div className="w-full flex gap-2 flex-wrap overflow-x-auto items-center bg-slate-100 border-slate-800 border-t p-2 ">
      <span className="">History:</span>
      {items.map((node, idx) => (
        <React.Fragment key={`${node.id}-${idx}`}>
          <button
            className="bg-slate-200 hover:bg-slate-300 rounded text-sm px-2 py-1"
